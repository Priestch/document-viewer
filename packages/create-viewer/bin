...
var src$1 = globalyzer$1;

const fs = fs$1;
const globrex = globrex_1;
const globalyzer = src$1;
const { join, resolve, relative } = path$1;
const isHidden = /(^|[\\\/])\.[^\\\/\.]/g;
...
async function main() {
  ...
  const templatesDir = path$1.join(path$1.parse(binPath).dir, "templates");
  const templates = fs$1
    .readdirSync(templatesDir)
    .filter((dir) => {
      return fs$1.statSync(path$1.join(templatesDir, dir)).isDirectory();
    })
    .map((dir) => {
      return {
        name: dir,
  ...
  const templateDir = path$1.join(templatesDir, templateName);

  const gitignore = lib$1.compile(gitIgnore);
  const files = sync("**/*", { cwd: templateDir }).filter(gitignore.accepts);

  mkdirp(projectName);
  files.forEach((file) => {
    const src = path$1.join(templateDir, file);
    const dest = path$1.join(projectName, file);
