...
use oxc::ast::ast::{BindingRestElement, FunctionBody, FunctionType, ImportDeclaration, ImportOrExportKind, ModuleExportName, PropertyKind, Statement, TSTypeAnnotation, TSTypeParameterDeclaration, VariableDeclarationKind};
...
pub struct UtilsGen<'a> {
    ...
    func_names_in_helper: allocator::Vec<'a, Atom<'a>>,
}
...
impl<'a> UtilsGen<'a> {
    pub fn new(allocator: &'a Allocator) -> Self {
        ...
        Self {
            ...
            func_names_in_helper: allocator::Vec::from_iter_in(vec![
    ...
    pub fn get_module_text(&mut self) -> String {
        let mut properties = allocator::Vec::with_capacity_in(self.func_names_in_helper.len(), self.builder.allocator);
        ...
        for name in &self.func_names_in_helper {
            ...
            let property = self.builder.object_property(
                SPAN,
                PropertyKind::Init,
                self.builder.property_key_identifier_name(SPAN, Atom::from(s)),
                self.builder.expression_identifier_reference(SPAN, Atom::from(s)),
                None,
                false,
                true,
                false,
            ...
            properties.push(
                self.builder.object_property_kind_from_object_property(property)
            )
        }
        ...
        let statement = self.builder.return_statement(
            SPAN,
            Some(
                self.builder.expression_from_object(
                    self.builder.object_expression(
                        SPAN,
                        properties,
                        None,
                    )
        ...
        self.helper_body.statements.push(
            self.builder.statement_from_return(statement)
        );

        let body = self.helper_body.clone_in(self.builder.allocator);
        let id = self.builder.binding_identifier(SPAN, Atom::from("createHelper"));
        let parameters = self.builder.formal_parameters(
            ...
            self.builder.vec1(self.builder.formal_parameter(
                ...
                self.builder.binding_pattern(
                    self.builder.binding_pattern_kind_binding_identifier(SPAN, "PDFViewerApplication"),
                    None::<TSTypeAnnotation>,
                    false,
                ),
                None,
        ...
        let export_stmt = self.builder.module_declaration_export_named_declaration(
            ...
            self.builder.vec1(
                self.builder.export_specifier(
                    SPAN,
                    ModuleExportName::IdentifierName(self.builder.identifier_name(SPAN, Atom::from("createHelper"))),
                    ModuleExportName::IdentifierName(self.builder.identifier_name(SPAN, Atom::from("createHelper"))),
    ...
    pub fn add_func_statement(&mut self, statement: Statement<'a>) {
        self.helper_body.statements.push(statement)
    }

    pub fn add_func_name(&mut self, name: Atom<'a>) {
        self.func_names_in_helper.push(name)
    }
